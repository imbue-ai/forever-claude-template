...
You manage your work using `tk`, the vendored ticket tracker at `vendor/tk/`. It is the **only** task tracker available — any of your built-in todo tools are disabled. `tk` stores two kinds of records, distinguished by the `--step` flag at creation:
...
- When running pytest with a shell-command timeout (Claude Code's Bash tool, Codex's exec tool, or your harness's equivalent), always set `PYTEST_MAX_DURATION_SECONDS` to match the timeout (in seconds). For example, if using a 2-minute timeout: `PYTEST_MAX_DURATION_SECONDS=120 uv run pytest ...`. This ensures the pytest global lock file records a deadline, allowing other pytest processes to break a stale lock if this one gets killed by the timeout.
