#!/usr/bin/env python3
"""Create one Modal workspace per (persona x trial) from the FCT autosender branch.

Runs INSIDE the minds-box container (via ``run-eval.sh`` / ``docker exec``): the create API
is the container-local Minds server and ``git_url`` must be a path Minds itself can read. For
each persona it makes a full local clone of the autosender base, slots the persona's config
into ``scripts/first_command.json``, commits it (only committed content reaches the sandbox via
git-mirror), and POSTs the create -- so every workspace lands in the one dashboard. The in-sandbox
chat-watcher then delivers each persona's ``first_prompt`` as the user, once its agent goes idle.
"""
...
def load_personas_from_obj(obj) -> list[dict]:
    ...
    for i, p in enumerate(personas):
        pid = _slug(str(p.get("id") or "persona-{}".format(i + 1)))
...
def ensure_base(repo: str, branch: str) -> None:
    ...
    # git creds); fall back to cloning here when the tester is run standalone.
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Create one Modal workspace per persona x trial.")
    ap.add_argument("personas", nargs="?", help="path to persona.json ([{id,persona,first_prompt}, ...])")
    ...
    if not args.personas:
        ap.error("persona.json path is required")
    if args.trials < 1:
