...
#
# Usage: create_reviewer_settings.sh [AUTOFIX_ENABLE] [CI_ENABLE] [VERIFY_CONVERSATION_ENABLE] [AUTOFIX_MINOR] [VERIFY_ARCHITECTURE_ENABLE] [STOP_HOOK_ENABLE] [FETCH_AND_MERGE_ENABLE]
#
...
#   FETCH_AND_MERGE_ENABLE    - stop_hook.fetch_and_merge (true when 1, false when 0)
...
FETCH_AND_MERGE_BOOL=$( [[ "$FETCH_AND_MERGE_ENABLE" == "1" ]] && echo true || echo false )
...
jq -n \
    ...
    --argjson fetch_and_merge "$FETCH_AND_MERGE_BOOL" \
    --arg prompt "$PROMPT" \
    '$existing * {
        ...
        "stop_hook": {"enabled_when": $stop_hook_enabled_when, "fetch_and_merge": $fetch_and_merge}
