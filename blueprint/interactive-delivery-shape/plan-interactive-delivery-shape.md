...
> * After mock confirmation, `build-web-service` foregrounds the build-to-a-usable-site (+ smoke verify) so the user can test it themselves, then — once the user confirms the working site looks generally right — self-spawns a background finalization worker for thorough Playwright tests, ratchets, `/autofix`, and the code guardian gates. The trigger is that explicit confirmation, never the agent's own sense of completeness; the main agent never runs the gates.
...
- **Pure web-service request** ("build me a dashboard"): the agent enters `build-web-service`, asks only blocking clarifications, proposes a small plan, and puts a throwaway mock in front of the user (served as a route inside the scaffolded service) — looping present → feedback → updated mock until the user explicitly confirms the UI shape. Only then does it build the site to a usable state, smoke-verify, and surface it. The user can immediately test it; the agent asks an explicit "does this generally look right?" and only on that confirmation spawns the background finalization worker (thorough testing + guardian gates). Two escalating-fidelity gates back this: the mock confirms the UX *shape* (cheap to pivot, pre-build); the working-site confirmation confirms the real *behavior* (pre-finalize) — the point where deep-architecture surprises surface, so finalizing earlier risks wasted hardening.
...
- Keep the existing Steps 1-4 (scaffold → implement → verify → surface) as the build/harden mechanics invoked after confirmation; clarify that the post-confirmation foreground work builds the site to a *usable* state plus the existing smoke verify, then stops and hands the running site to the user to try.
- Define the **finalization trigger** explicitly: once the usable site is in front of the user, the agent asks a plain "this generally looks good?" and only backgrounds finalization on that confirmation — never on the agent's own judgment that the code looks done (agent-judged completeness ≠ user-confirmed behavior). Each surface-level change request on the working site is a cheap foreground iteration that resets the clock (the main agent has run no gates/thorough tests yet, so pivots stay cheap); finalization fires only when the user's response is a confirmation rather than a change.
- Add guidance on reading the confirmation signal: if the user starts asking for surface-level (cosmetic) tweaks, or pivots to a slightly unrelated task or follow-up, treat that as a sign the core is settled — still ask for confirmation, but ground it in "seems like we've got the core thing settled here — good to lock it in?" rather than leaving it open-ended. Keep waiting for an explicit confirmation rather than firing on a timeout or silence; the user is never blocked because they already hold the usable site.
- Add the always-on background finalization handoff (gated on the confirmation above): self-spawn the bundled worker sub-skill via `--template subskill-worker` for thorough Playwright testing, ratchets, `/autofix`, and the guardian gates. State in prose that the main agent never runs the gates / thorough passes.
