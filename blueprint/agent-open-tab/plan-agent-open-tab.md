...
- Add a new loopback-only HTTP endpoint on the workspace_server (`apps/system_interface/imbue/minds_workspace_server/server.py`) -- e.g. `POST /api/open-tab/<name>/broadcast` -- that mirrors the surviving refresh-service broadcast endpoint: validates loopback caller, validates the service is registered in `runtime/applications.toml`, then calls `broadcaster.broadcast_open_tab(name)`. No event-file write, no request-events plumbing.
