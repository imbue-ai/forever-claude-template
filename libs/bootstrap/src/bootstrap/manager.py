...
def _find_existing_chat_agent_id(host_name: str) -> str | None:
    ...
    subsequent boot, so the retry must adopt the survivor instead. The lookup
    passes --active so an archived leftover (or one on a dead host) is never
    adopted as the welcome-resend target. Returns None on lookup failure or
    ambiguity, keeping the caller on the plain create path.
    """
