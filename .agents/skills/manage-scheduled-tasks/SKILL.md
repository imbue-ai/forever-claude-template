...
`/<skill>`, so the skill runs fresh; the agent surfaces its own chat tab at
run start via `scripts/layout.py open` (the same way web apps are surfaced).
Pass `--template <t>` only when you want a custom
