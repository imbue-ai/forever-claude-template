...
`runtime/update/*/task.md` (the glob matches exactly one file -- each
worker handles a single task). At the start of your run, validate
its frontmatter and extract the three required fields with:
...
It prints three shell-evalable `KEY=value` lines on stdout
(`LEAD_AGENT=`, `LEAD_REPORT_DIR=`, `TRANSCRIPT_PATH=`) and exits
non-zero with a stderr message if any required field is missing, not
a string, or empty. The first two address reports back to the lead;
`transcript_path` is where Stage 1's incident transcript (or
committed diff) lives.
