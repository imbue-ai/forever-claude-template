...
- Note that tasks are *not* allowed to finish without A) all tests passing (in CI, if a PR exists), B) running /autofix to verify and fix code issues, and C) running /verify-conversation to review the conversation for behavioral issues.
