...
Supervisord is for **long-running** processes (the mind's apps and always-on daemons). For **routine jobs that run on a schedule and then exit**, use cron/anacron instead (see below) -- do not write a sleep-loop supervisord program for periodic work.

# Scheduled tasks

For recurring routines that are part of *the mind itself* -- backups, health checks, daily messages to the user, the nightly Caretaker (on by default), and anything else that should happen on a cadence rather than run continuously -- use **cron/anacron**, not supervisord. (Apps and services *within* the mind, which stay running, are supervisord programs; routines the mind performs *on a cadence* are cron/anacron jobs.)

Recurring jobs use the stock OS tools, and you choose per job: **anacron** (`/etc/anacrontab`) for jobs that run about once a day or coarser and **must not be skipped** when the container was off -- anacron catches up missed runs at the next opportunity, coalescing several misses into one run; **cron** (`/etc/cron.d/` drop-ins) for precise times or sub-daily cadences -- exact but never backfilled. The cron daemon runs under supervisord (`[program:cron]`); anacron is triggered once per boot (`[program:anacron-boot]`, the catch-up path) and hourly via `/etc/cron.d/fct-anacron`. Cron and anacron scrub the job environment, so prefix every job command with `scripts/with_agent_env.sh` (it restores the workspace env from the snapshot the bootstrap writes each boot and runs from the repo root).

The container's timezone is set to the user's local timezone at each boot (the bootstrap pulls it from the minds app through the latchkey gateway). When the user asks to schedule something, first re-check their current timezone against the minds API and update `/etc/localtime` + `/etc/timezone` (then `supervisorctl restart cron`) if it changed. See the **`manage-scheduled-tasks`** skill for the full recipe, entry formats, and the timezone check.

**Scheduled *agent* tasks.** A scheduled job can do more than run a script -- it can wake an agent that runs a skill on a cadence, in its own chat tab. Use **`scripts/run_task_agent.sh <skill>`** as the job command: it spawns a persistent singleton agent for that skill (labelled `task_agent=<skill>`), and on each run clears its chat and re-sends `/<skill>` so the skill runs fresh. The nightly Caretaker is the built-in example (the `caretaker` line in `/etc/anacrontab`); a new one (e.g. a morning news agent) needs only a skill at `.agents/skills/<name>/SKILL.md` plus an anacron or cron entry running `scripts/run_task_agent.sh <name>` through the env wrapper -- no new agent template required. When the user wants something done on a regular cadence by an agent, reach for this.

Rule of thumb: if it's an *app or service the mind exposes* (a web app, an always-listening bot), make it a supervisord program. If it's a *routine the mind performs on a cadence* (back something up, run a check, message the user each morning, the Caretaker's nightly run), make it a cron/anacron job.
