...
- **Feedback systems combine binary and free-form signals.** When building anything that learns from user feedback, always include *both* a basic binary signal (thumbs up/down, good/bad, keep/skip, or whatever fits the context) *and* free-form text input plus an LLM judge -- unless the user specifies a particular feedback mechanism, in which case follow their description and override this default. Binary feedback is low-friction and gives you a usable signal even when the user doesn't feel like typing; free-form text captures the nuance you can't anticipate (rules like "never surface this sender", qualitative explanations like "this was important because X", or ad-hoc emoji tags). The exact manifestation depends on what's being built, but both should be present and intuitively accessible. Avoid prescribing fixed schemas or rigid taxonomies beyond the binary signal before you know what kind of qualitative feedback the user will actually give -- switch to a more structured pipeline later only if data volume or latency genuinely requires it.
