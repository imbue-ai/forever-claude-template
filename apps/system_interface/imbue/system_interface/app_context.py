...
class SystemInterfaceState(MutableModel):
    ...
    chat_file_timestamps: ChatFileTimestampStore
    ...
    def get_or_create_watcher(self, agent_info: AgentInfo) -> AgentSessionWatcher:
        ...
        with self._watchers_lock:
            ...
            def on_events(agent_id: str, events: list[dict[str, Any]]) -> None:
                ...
                # Fingerprint any files the new messages reference as close
                # to post time as possible (the stat happens on the store's own
                # worker thread, so the fan-out is not blocked on file I/O).
                self.chat_file_timestamps.enqueue_events(events)
    ...
    def shutdown(self) -> None:
        ...
        self.chat_file_timestamps.stop()
        try:
