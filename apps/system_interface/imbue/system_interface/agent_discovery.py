...
def send_message(agent_name: str, message: str) -> bool:
    ...
    mngr_ctx, cg = _get_mngr_context()
    try:
        # mngr's message API takes a pre-resolved set of agents, so resolve the
        # name-or-id to live agents first (mirroring `mngr message`). A name or
        # ID that matches nothing raises AgentNotFoundError, which we treat as
        # "no agent to message" and report as a failed send.
        address = AgentAddress(agent=parse_agent_name_or_id(agent_name))
        try:
