...
# watcher uses ``IGNORE`` via broadcast_all_ignored -- but the default behavior
# is ``STORE``, so any future caller that omits ``buffer_behavior`` would
# otherwise grow it without bound for the agent's lifetime). When the cap is
# exceeded the oldest events are dropped; late-joining subscribers replay only
